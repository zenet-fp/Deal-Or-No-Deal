...
class DealOrNoDeal:
    def __init__(self):
        self._available_boxes = [0.01, 0.1, 1, 5, 25, 50, 100, 250, 500, 1000, 2500, 5000, 7500, 12500, 25000, 50000,
    ...
    def dealer_choice(self):
        self.valid_boxes = [x for x in self._available_boxes if x != "GONE"]
        expected_value = sum(self.valid_boxes) / len(self.valid_boxes)
        if 16 <=len(self.valid_boxes) <= 18:
            deal = math.ceil(0.4 * (expected_value))
            print(deal)
        elif 10 <= len(self.valid_boxes) <= 15:
            deal = math.ceil(0.7 * (expected_value))
            print(deal)
        elif 6 <= len(self.valid_boxes) <= 10:
            deal = math.ceil(expected_value)
            print(deal)
